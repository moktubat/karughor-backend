...
export const getDashboardStats = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const [totalOrders, todayOrders, deliveredOrders, totalRevenue, pendingOrders, lowStockProducts] =
            await Promise.all([
    ...
    try {
        ...
        switch (period) {
            case 'today':
                ...
                break;
            case 'week':
                dateFilter = { orderDate: { $gte: new Date(now.setDate(now.getDate() - 7)) } };
                break;
            case 'month':
                dateFilter = { orderDate: { $gte: new Date(now.setMonth(now.getMonth() - 1)) } };
                break;
            case 'year':
                dateFilter = { orderDate: { $gte: new Date(now.setFullYear(now.getFullYear() - 1)) } };
                break;
        }

        const [revenue, dailySales, topProducts] = await Promise.all([
            Order.aggregate([
